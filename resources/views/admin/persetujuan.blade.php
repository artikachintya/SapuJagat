...
<div class="app-content">
    ...
    <div class="row">
        ...
        <div class="info-box">
            ...
            <span class="info-box-text">Transaksi Disetujui</span>
                    ...
                    <table id="jenis-sampah" class="table table-striped align-middle" style="background-color:black;">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    Request Pengguna: {{ $order->date_time_request }}<br>
                                    Jadwal Pickup: {{ $order->pickup_time }}<br>
                                    Penjemput mulai: {{ $order->pickup->start_time ? $order->pickup->start_time : "Belum"}}<br>
                                    Sampah Dijemput: {{ $order->pickup->pick_up_date ? $order->pickup->pick_up_date : "Belum"}}<br>
                                    Sampah Tiba: {{ $order->pickup->arrival_date ? $order->pickup->arrival_date : "Belum"}}<br>
                                </td>
                                ...
                                <td>
                                    @foreach($order->details as $detail)
                                        {{ $detail->trash->name ?? 'Jenis Tidak Diketahui' }}: {{ $detail->quantity }} kg<br>
                                    @endforeach
                                </td>
                                <td>
                                ...
                                <!-- Modal -->
                                <div class="modal fade" id="detailModal{{ $order->order_id }}" tabindex="-1" aria-labelledby="modalLabel{{ $order->order_id }}" aria-hidden="true">
                                    <div class="modal-dialog modal-dialog-scrollable modal-lg">
                                        <div class="modal-content rounded-4">
                                            <div class="modal-header bg-success text-white rounded-top-4">
                                                <h5 class="modal-title" id="modalLabel{{ $order->order_id }}">Detail Order #{{ $order->order_id }}</h5>
                                            ...
                                            <div class="modal-body text-dark bg-light">
                                                ...
                                                <div class="row mb-3">
                                                    <div class="col-md-6">
                                                        ...
                                                        <p><strong>Admin:</strong> {{ $order->approval->user->name ?? '-' }}</p>
                                                        <p><strong>Pengguna:</strong> {{ $order->user->name }}</p>
                                                        <p><strong>Penjemput:</strong> {{ $order->pickup->user->name }}</p>
                                                        <p><strong>Tanggal/Waktu Permintaan:</strong> {{ $order->pickup->start_time ?? '-' }}, {{ $order->pickup->pick_up_date ?? '-' }}</p>
                                                        <p><strong>Tanggal/Waktu Penjemputan:</strong> {{ $order->pickup->pick_up_date ?? '-' }}</p>
                                                        <p><strong>Tanggal/Waktu Selesai:</strong> {{ $order->pickup->arrival_date ?? '-' }}</p>
                                                        <p><strong>Tanggal/Waktu Respon Admin Terbaru:</strong> {{ $order->approval->date_time ?? '-' }}</p>
                                                    </div>

                                                    <div class="col-md-6">
                                                        <p><strong>Jenis Sampah:</strong>
                                                            @foreach($order->details as $detail)
                                                                {{ $detail->trash->name }}: {{ $detail->quantity }} KG<br>
                                                            @endforeach
                                                        </p>
                                                        @php $total = 0; @endphp
                                                        @foreach($order->details as $detail)
                                                        ...
                                                        @endforeach
                                                        <p><strong>Berat Sampah:</strong> {{ $order->details->sum('quantity') }} KG</p>
                                                        <p><strong>Harga Total:</strong> Rp {{ number_format($total, 0, ',', '.') }}</p>
                                                        <p><strong>Notes Penjemput:</strong></p>
                                                        <div class="bg-success-subtle text-dark p-2 rounded">{{ $order->pickup->notes ?? '-' }}</div>
                                                        ...
                                                        @php
                                                            ...
                                                            if ($order->approval) {
                                                                switch ($order->approval->approval_status) {
                                                                    case 0:
                                                                        $statusText = 'Ditolak';
                                                        ...
                                                        <div class="p-2 rounded fw-bold {{ $statusColor }}">
                                                            {{ $statusText }}
                                                        </div>
                                                ...
                                                <div class="row mb-4 text-center">
                                                    <div class="col-md-6">
                                                        <p class="fw-bold">Bukti Pengguna</p>
                                                        @if ($order->photo)
                                                            <img src="{{ asset('storage/' . $order->photo) }}" class="img-fluid rounded shadow" style="max-height: 200px;">
                                                        @else
                                                            <p>Tidak ada foto</p>
                                                        @endif
                                                    </div>
                                                    <div class="col-md-6">
                                                        <p class="fw-bold">Bukti Penjemput</p>
                                                        @if ($order->pickup_photos)
                                                            <img src="{{ asset('storage/' . $order->pickup_photos) }}" class="img-fluid rounded shadow" style="max-height: 200px;">
                                                        @else
                                                            <p>Tidak ada foto</p>
                                                        @endif
                                                    </div>
                                                ...
                                                @if ($order->approval)
                                                    <hr>
                                                    <p><strong>Respon Admin Sebelumnya:</strong></p>
                                                ...
                                                <form action="{{ route('admin.persetujuan.store') }}" method="POST">
                                                    @csrf
                                                    ...
                                                    <div class="mb-3">
                                                        <label for="notes" class="form-label fw-bold">Tulis Respon Admin:</label>
                                                        <textarea name="notes" class="form-control bg-success-subtle" rows="4" placeholder="Tuliskan Respon" required></textarea>
                                                    </div>
